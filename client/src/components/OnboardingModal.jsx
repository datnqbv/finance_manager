...
const OnboardingModal = ({ onClose }) => {
  ...
  const handleCreateCategories = async () => {
    setCreating(true);
    const toCreate = DEFAULT_CATEGORIES.filter((_, i) => selected.has(i));
        ...
        {step === 1 && (
          <div className="p-6">
            <div className="flex items-center gap-2 mb-1">
              ...
              <h2 className="text-lg font-black text-gray-900 dark:text-white">Chọn danh mục</h2>
            </div>
            <p className="text-xs text-gray-400 dark:text-gray-500 mb-4 ml-9">Chọn các danh mục bạn muốn tạo. Bạn có thể thêm/sửa sau.</p>

            {/* Income */}
            <p className="text-xs font-bold text-emerald-600 dark:text-emerald-400 mb-2 uppercase tracking-wide">Thu nhập</p>
            <div className="grid grid-cols-2 gap-2 mb-4">
              {DEFAULT_CATEGORIES.filter(c => c.type === 'income').map((cat, idx) => {
            ...
            <div className="flex gap-3">
              ...
              >
                {creating ? (
                  ...
                  <>
                    <FiZap size={15} /> Tạo {selected.size} danh mục
                  </>
                )}
              </button>
            </div>
