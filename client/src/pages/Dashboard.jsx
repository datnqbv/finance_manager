        ...
        <div className="grid grid-cols-1 gap-5 xl:grid-cols-12 items-stretch">
          ...
          <div className="xl:col-span-8 flex flex-col gap-5">
            ...
            <div className="rounded-xl bg-white shadow-sm dark:bg-[#191d25] flex-1 flex flex-col">
                ...
                <table className="min-w-full text-sm table-fixed">
                  <thead>
                    <tr className="border-b border-[#eceff4] text-left text-xs font-bold uppercase tracking-wider text-[#7a808c] dark:border-[#2b313d] dark:text-[#9fa7b4]">
                      <th className="px-5 py-3 w-1/4">{isEnglish ? 'Description' : 'Mô tả'}</th>
                      <th className="px-5 py-3 w-1/4">{t('category')}</th>
                      <th className="px-5 py-3 w-1/4">{t('date')}</th>
                      <th className="px-5 py-3 w-1/4 text-right">{t('amount')}</th>
                    </tr>
                  </thead>
                  <tbody>
                    {recentTransactions.length > 0 ? recentTransactions.map((tx, idx) => {
                      ...
                      <tr>
                        <td colSpan={4} className="px-5 py-10 text-center text-sm text-[#6f7480] dark:text-[#a4acba]">
              ...
              <div className="block md:hidden divide-y divide-gray-100 dark:divide-[#242a35] flex-1 flex flex-col justify-center">
                {recentTransactions.length > 0 ? (
                  ...
                  <div className="py-8 text-center text-sm text-gray-500 flex-grow flex items-center justify-center">
                    {isEnglish ? 'No matching transactions.' : 'Không có giao dịch phù hợp.'}
                  </div>
                )}
              </div>
              ...
              {!isVipActive && (
                <div className="absolute inset-0 bg-white/70 dark:bg-[#191d25]/75 z-10 flex flex-col items-center justify-center p-6 text-center backdrop-blur-[2px]">
                  ...
                  <button
                    ...
                    className="mt-4 inline-flex items-center gap-1.5 rounded-xl bg-amber-500 hover:bg-amber-600 text-white font-extrabold px-4 py-2 text-xs transition-colors shadow-md shadow-amber-500/10"
                  >
                    <FiAward /> {isEnglish ? 'Upgrade to VIP' : 'Nâng cấp VIP'}
                  </button>
                </div>
              )}
              <div>
